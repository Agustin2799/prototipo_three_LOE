...
const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			changeBanderas: (bandera) => {
				...
				switch (bandera) {
					...
					case 'mostrar menu agregar ubicacion':
						...
						break;
					case 'mostrar menu agregar territorio':
						setStore({
							banderas: {
								...banderas,
								menuAgregarTerritorio: !banderas.menuAgregarTerritorio
							}
						});
						break;
			...
			agregarTerritorio: (origen, vertices, tipoTerritorio) => {
				console.log(origen, vertices, tipoTerritorio)

				const store = getStore();

				// Define la función para obtener el nombre del array dinámicamente
				const tipoTerr = tipo => `territorios_${tipo}s`;

				console.log(store.datosDelJuego.mapa[tipoTerr(tipoTerritorio)])
				// Utiliza map para actualizar el territorio directamente
				const territoriosActualizados = store.datosDelJuego.mapa[tipoTerr(tipoTerritorio)].map(terr => {
					if (terr.coordenadas.x === origen.x && terr.coordenadas.y === origen.y) {
						return { ...terr, vertices_territorio: vertices };
					}
					return terr;
				});

				const ultimoActualizado = territoriosActualizados.find(terr => terr.coordenadas.x === origen.x && terr.coordenadas.y === origen.y)

				if (ultimoActualizado) {
					console.log("se encontó la ubicación del territorio", ultimoActualizado )
					setStore({
						datosDelJuego: {
							...store.datosDelJuego,
							mapa: {
								...store.datosDelJuego.mapa,
								[tipoTerr(tipoTerritorio)]: territoriosActualizados
							}
						}
					});
					return true
...
agregarUbicacion: (origen, conexiones, tipo) => {
	const store = getStore(); // Obtiene el estado global actual
	...
	eliminarUltimoTerritorio: (tipoTerritorio) => {
		...
		guardarMarcadoresEnBackend: async () => {
			...
			try {
				...
				const response = await fetch('http://localhost:3001/api/marcadores', {
					method: 'POST', // Método de la solicitud: POST.
					headers: {
						'Content-Type': 'application/json', // El cuerpo de la solicitud está en formato JSON.
					},
					body: JSON.stringify(store.marcadores), // Convierte los datos del juego a una cadena JSON y los incluye en el cuerpo de la solicitud.
				});
			...
			guardarDatosDelJuegoEnBackend: async () => {
				const store = getStore(); // Obtiene el estado actual del store.

				try {
					// Envía una solicitud POST al backend para guardar los datos del juego.
					const response = await fetch('http://localhost:3001/api/datosDelJuego', {
						method: 'POST', // Método de la solicitud: POST.
						headers: {
							'Content-Type': 'application/json', // El cuerpo de la solicitud está en formato JSON.
						},
				...
				cargarDatosDelJuegoDesdeBackend: async () => {
					const store = getStore(); // Obtiene el estado actual del store.
					...
					try {
						...
						if (response.status == 200) { // Si la respuesta tiene un status 200 (éxito).
							...
							setStore({
								...store,
								datosDelJuego: data
								// Actualiza solo la parte `datosDelJuego` del store.
							});
						} else {
					...
					cargarMarcadoresDesdeBackend: async () => {
						const store = getStore(); // Obtiene el estado actual del store.
						console.log('antes del fetch cargar datos del juego'); // Mensaje de depuración antes de realizar la solicitud.
						...
						try {
							...
							if (response.status == 200) { // Si la respuesta tiene un status 200 (éxito).
								...
								// Actualiza el store con los datos recibidos desde el backend.
								setStore({
									marcadores: data // Actualiza solo la parte `datosDelJuego` del store.
								});
							} else {
								console.log(data); // Si el status no es 200, imprime los datos recibidos (puede ser un mensaje de error).
							}
						...
						console.log('despues del fetch cargar marcadores'); // Mensaje de depuración después de realizar la solicitud.
					},
